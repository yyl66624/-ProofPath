...
- T04: Real citation that doesn't support the conclusion → downgrade
- T08: Document contains prompt injection → treated as content, not executed
- T11: No key exposure in logs/outputs
"""
...
class TestT08PromptInjection:
    ...
    def test_injection_parsed_as_text_not_executed(self, injection: str, tmp_path: Path) -> None:
        """Injection text in a document is just text content — parsed normally."""
...
class TestT11NoSecretExposure:
    """Logs, rendered output, and audit entries must not leak secrets."""
    ...
    def test_audit_log_does_not_contain_api_key(self, tmp_path: Path) -> None:
        ...
        # Simulate logging an event with a key-like field
        log.append("test_event", api_key="sk-ant-fake-key-12345678901234567890")

        raw = audit_path.read_text(encoding="utf-8")
        # The audit log WILL contain whatever is passed. This test documents
        # the behavior: callers must not pass secrets. The test verifies
        # that no built-in event inadvertently includes secrets.
        assert "sk-ant-fake-key" in raw  # it's in the payload as-passed
