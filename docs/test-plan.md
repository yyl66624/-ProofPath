...
本文件保留 P09 启动时的 132 项测试基线及待补用例设计；下文的“待新增”和覆盖率表是**原始计划快照**，不能当作当前执行状态。四个新增测试文件及集成后的实测结果、T01–T12 最新判定，以 `docs/test-report.md` 为准。尤其不能把文本解析测试等同于模型抗提示注入，也不能把干跑操作闸门等同于真实网页提交。
...
覆盖状态：**原计划大部分覆盖；新增空/损坏/无文字层等解析测试，当前状态见测试报告**。
...
覆盖状态：**部分覆盖**。新增 429、连接失败、模型拒答及空响应测试；超时后输入保留、截断 JSON 等仍待验证。
...
| 层次 | 测试函数 | 状态 |
|---|---|---|
| 解析与核验 | `TestT08PromptInjection` | ⚠️ 仅验证文本解析及伪造引用拒绝 |

覆盖状态：**部分覆盖**。六种注入文本已被解析，但没有进入真实模型调用；不能据此声称模型未服从注入。
...
| T10-3 | `test_document_access_requires_authorization` | 文档 ID 即使可预测，也不能绕过用户身份与授权读取。 |

**现有相关覆盖**：`TestLoadDocument::test_doc_id_is_content_addressed` 仅验证内容寻址。内容哈希 ID 可由已知文件内容推算，不是访问控制或隔离机制。
...
覆盖状态：**部分覆盖**。新增测试验证 `api_key` 和嵌套 `authorization` 在审计日志落盘前脱敏；自由文本、全部错误路径和仓库文件仍需单独检查。

**待新增**：

| 编号 | 测试名称 | 说明 |
|---|---|---|
| T11-1 | `test_audit_log_does_not_contain_api_key` | ✅ 结构化敏感字段落盘前脱敏；不能推导所有自由文本绝无密钥。 |
...
覆盖状态：**部分覆盖**。已验证干跑执行器前的阻断与确认闸门；真实浏览器页面跳转、拒绝后停止和提交仍待集成测试。
...
| 凭证构建与 429 等 API 错误 | `test_constructor_wraps_credential_exception`、`test_api_status_error` | ✅ 部分覆盖；不等于 401/529 分类全覆盖 |
| 连接失败 | `test_api_connection_error` | ✅ 已覆盖；超时与输入保留仍待补 |
...
| 产品 `check` 真实模型调用 | — | ❌ 未验证；另见 `docs/model-verification.md` 的脚本级 3 次调用 |

### 3.5 模型质量评估（Model Quality Evaluation）

当前状态：P09 正式质量评估仅有 ScriptedReasoner 模拟测试；主线另有 3 次同一样例的 DeepSeek 脚本级接线验证，不构成 20 例质量评估。
...
| 场景 | 差距 | 工作量估计 |
|---|---|---|
| T08 | 已补文本解析和伪造引用拒绝；真实模型服从注入未测 | 推理链路对抗集 + 人工复核 |
| T07 | 已补 429/连接异常等；超时输入保留、截断 JSON 未测 | 异常恢复与重试测试 |
| T06 | 已补损坏 PDF 和无文字层；OCR 真实路径未测 | 安装可选依赖后集成测试 |
| T11 | 已补结构化审计字段脱敏；自由文本/错误信息/仓库级扫描未全部验证 | 分层密钥泄露测试 |
...
| T10 | 无多用户体系，无隔离需求 |
| UI 检查 | 无前端实现 |
| 产品真实模型端到端 | P06 尚未接线；脚本级调用见 `docs/model-verification.md` |
